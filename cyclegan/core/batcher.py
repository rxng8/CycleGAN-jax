...
class Batcher:
  ...
  def __init__(
    ...
    """Make the generator batch-able

    Args:
        sources (List[Generator]): List of generator/function that return an iterator object. E.g.,
          ```python
            def generator():
            while True:
              yield {k: datum}
          ```
          `batcher = Batcher(sources)`
          In this case, generator MUST yield a DICTIONARY of data
          In this case `sources = [generator] * 10` if the batch size is 10
          Then, in the actual run, we call batcher() to return an
            iterator. e.g., `it = batcher()`, we then can do `next(it)`
        workers (int, optional): _description_. Defaults to 0.
        postprocess (_type_, optional): _description_. Defaults to None.
        prefetch_source (int, optional): _description_. Defaults to 4.
        prefetch_batch (int, optional): _description_. Defaults to 2.
    """
